...
const OUT_OF_SCOPE_LD_JSON_HTML = `<!doctype html><html><head>
<script type="application/ld+json">{"@context":"https://schema.org","@type":"ApartmentComplex","name":"Kinetic","address":{"@type":"PostalAddress","addressCountry":"US","addressLocality":"Atlanta","addressRegion":"GA","postalCode":"30309","streetAddress":"1025 Spring Street"}}</script>
</head><body></body></html>`
...
describe('extractPropertyFacts (deterministic path)', () => {
  ...
  it('returns facts with an out-of-scope city unchanged (scope check is the caller\'s job)', async () => {
    const facts = await extractPropertyFacts(OUT_OF_SCOPE_LD_JSON_HTML, 'https://example.com/', {
      geocode: async () => ({ latitude: 33.8, longitude: -84.4 }),
    })
    expect(facts?.city).toBe('Atlanta')
  })
