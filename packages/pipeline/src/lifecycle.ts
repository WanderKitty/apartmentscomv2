...
// Confirm/stale/gone ladder (spec §5.2/§5.4). A scrape that finds a
// listing again (hash-unchanged short-circuit included) confirms it's
// still live; a listing that stops appearing in a source's payload gets
// one cycle of grace (active → stale) before being marked gone.

/** Hash short-circuit (spec §5.2): confirms every non-gone listing of a source is still live. */
...
/**
 * One-cycle grace (spec §5.4): listings of this source not present in the
 * current scrape's collapse keys go active → stale, then stale → gone on
